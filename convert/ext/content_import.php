...
if ($offset >= $total) {
	...
	if (!$now_modelid) ext_go('开始导入会员数据。<br>内容数据导入完成。', 'filename=member_model');
	$total = count_model_total($now_modelid);
	$page = 1;
	$offset = 0;
}
...
ext_go('正在导入'.$dede_models[$now_modelid]['typename'].'('.($offset < $total ? ($offset >= 0 ? (($pagesize*$page)>=$total ? $total : ($pagesize*$page)) : $pagesize) : 0).'/'.$total.')', 'filename=content_import&now_modelid='.$now_modelid.'&total='.$total.'&page='.($page+1));